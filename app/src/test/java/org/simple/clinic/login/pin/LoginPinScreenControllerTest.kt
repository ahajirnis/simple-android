...
class LoginPinScreenControllerTest {
  ...
  fun `when screen starts, show phone number`() {
    whenever(userSession.ongoingLoginEntry()).thenReturn(Single.just(OngoingLoginEntry(uuid = UUID.randomUUID(), phoneNumber = "123")))
  ...
  fun `when PIN is submitted, request login otp and open home screen`() {
    val ongoingLoginEntry = OngoingLoginEntry(uuid = UUID.randomUUID(), phoneNumber = "9999")
    ...
    verify(userSession).saveOngoingLoginEntry(OngoingLoginEntry(uuid = ongoingLoginEntry.uuid, phoneNumber = "9999", pin = "0000"))
  ...
  fun `if request otp api call throws any errors, show errors`() {
    val ongoingEntry = OngoingLoginEntry(uuid = UUID.randomUUID(), phoneNumber = "99999")
    whenever(passwordHasher.compare(any(), any())).thenReturn(Single.just(PasswordHasher.ComparisonResult.SAME))
